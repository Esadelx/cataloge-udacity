...
def showLogin():
    ...
    'border-radius: 150px;-webkit-border-radius: 150px;-moz-border-radius: 15'
    '0px;"> '
...
def editCatalog(catalog_id):
    ...
    if editedCatalog.user_id != login_session['user_id']:
        ...
        "myFunction() {alert('You"
        " are not authorized to edit"
        " this catalog. Please create"
        " your own catalog in order to"
...
def deleteCatalog(catalog_id):
    ...
    if catalogToDelete.user_id != login_session['user_id']:
        ...
        "{alert('You are not authorized to delet"
        "e this catalog. Please create your own "
        "catalog in order to"
...
# Show a Catalog Items with validate
...
# Create a new item with validate
...
def editCatalogItem(catalog_id, item_id):
    ...
    if login_session['user_id'] != catalog.user_id:
        return "<script>function myFunction()"
        " {alert('You are not authorized to ed"
        "it item items to this catalog. Please"
        " create your own catalog in order to "
    ...
    if request.method == 'POST':
        ...
        catalog1 = session.query(Catalog).filter_by(
        ...
        if catalog1 == catalog:
            ...
            session.commit()
            newitem = Items(
                name=editedItem1.name,
                description=editedItem1.description,
                price=editedItem1.price,
                picture=editedItem1.picture,
                catalog=catalog1)
            session.add(newitem)
            session.commit()
        ...
        return redirect(
            url_for(
                'showItems',
                catalog_id=editedItem1.catalog_id))
    else:
        catalogs = session.query(Catalog).all()
        return render_template(
...
def deleteCatalogItem(catalog_id, item_id):
    ...
    if login_session['user_id'] != restaurant.user_id:
        return "<script>function myFunction()"
        " {alert('You are not authorized to del"
        "ete catalog items to this catalog. Plea"
        "se create your own catalog in order to "
